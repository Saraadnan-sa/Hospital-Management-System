...
void doctor::saveMap()
{
    ...
    f << "doctorId,firstName,lastName,gender,age,mobNumber,address,type,appointmentsBooked,fees\n";
    for (auto i : hospital::doctorsList)
        f << i.second.id << "," << i.second.firstName << "," << i.second.lastName << "," << i.second.gender
          ...
          << "," << i.second.type << "," << i.second.appointmentsBooked <<"," << i.second.fees << endl;
    f.close();
...
void doctor::addPerson()
{
    ...
    while (true) {
        try {
            cout << "Enter the fees of doctor:\n";
            
            
            fees=0;
            if (fees < 0|| !(cin >> fees)) {
                throw invalid_argument("Enter valid fees.");
            }
