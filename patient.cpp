...
void patient::addPerson()
{
    ...
    while (true) {
        try {
            ...
            height=0;
            if (height < 0|| !(cin >> height)) {
                throw invalid_argument("Enter valid height.");
            }
    ...
    while (true) {
        try {
           ...
           // cin >> weight;
            weight=0;
            if (weight < 0|| !(cin >> weight)) {
                throw invalid_argument("Enter a Valid weight.");
            }
...
void patient::reportADeath()
{
    ...
    remove("./data/patients.csv");
    rename("./data/temp.csv", "./data/patients.csv");
    cout << firstName << " " << lastName << " reported dead\n";
    return;
}
